...
export default function TravelCheckOutCompleteInfo({ tour }) {
  ...
  return (
    <Stack spacing={5}>
      ...
      <Stack spacing={2} direction={{ xs: 'column', md: 'row' }} justifyContent={{ md: 'space-between' }}>
        <Stack spacing={2}>
      ...
      >
        ...
        <LineItem icon={<Iconify icon={calendarIcon} />} label='Departure day' value={fDate(new Date())} />
        ...
        <LineItem icon={<Iconify icon={calendarIcon} />} label='Booking day' value={fDate(new Date())} />
      ...
      <Stack spacing={2.5} direction={{ xs: 'column', md: 'row' }} justifyContent='center'>
        <NextLink href='/' passHref>
          <Button variant='outlined' size='large' color='inherit' startIcon={<Iconify icon={chevronLeft} />}>
            Back Home
          </Button>
        </NextLink>
  ...
  return (
    <TextIconLabel
      ...
      value={
        <>
          {label}
          <Typography variant='subtitle2' sx={{ color: 'text.primary', flexGrow: 1, textAlign: 'right' }}>
            {value}
          </Typography>
        </>
      }
