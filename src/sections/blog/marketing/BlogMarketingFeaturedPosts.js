...
// components
import { Image, CarouselDots, CarouselArrows, TextMaxLine } from '../../../components'
        ...
        (post, index) =>
          selected === index && (
            <Box key={post.slug} sx={{ position: 'absolute', top: 0, width: 1, height: 1 }}>
              <Image alt='post cover' src={post.frontmatter.coverImg} sx={{ width: 1, height: 1 }} />
            </Box>
          )
      )}
    </RootStyle>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} sx={{ bgcolor: 'background.default' }}>
      ...
      >
        <Stack spacing={1}>
          ...
          <NextLink passHref as={Routes.marketing.post(slug)} href={Routes.marketing.post('[slug]')}>
