...
import { getAllPosts, getPostData, getAllPostSlugs } from '../../../src/utils/get-mardown/career/posts'
// config
...
import { Markdown, Breadcrumbs } from '../../../src/components'
// sections
...
export default function CareerPostPage({ post, posts }) {
  ...
  const { title, description, duration, createdAt, favorited, author, shareLinks, tags } = frontmatter
    ...
    <Page title={`${title} - Post | Career`}>
      <RootStyle>
        ...
        <Container>
          <Breadcrumbs
            sx={{ my: 3 }}
            links={[{ name: 'Home', href: '/' }, { name: 'Blog', href: Routes.career.posts }, { name: title }]}
          />
        </Container>
